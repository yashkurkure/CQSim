...
class Job_trace:
    ...
    def import_job_file(self, job_file):
        ...
        while i < self.read_num or self.read_num <= 0:
            ...
            if j >= self.anchor:
                ...
                tempInfo = {
                    "id": int(temp_dataList[0]),
                    "submit": self.density * (float(temp_dataList[1]) - min_sub)
                    + temp_start,
                    "wait": float(temp_dataList[2]),
                    "run": float(temp_dataList[3]),
                    "usedProc": int(temp_dataList[4]),
    ...
    def scoreCmp(self, jobIndex_c1, jobIndex_c2):
        return -self.cmp(
            self.jobTrace[jobIndex_c1]["score"], self.jobTrace[jobIndex_c2]["score"]
        )
    ...
    def job_start(self, job_index, time):
        ...
        self.debug.debug(
            ...
            + "]"
            + " Req:"
            + str(self.jobTrace[job_index]["reqProc"])
            + " Run:"
            + str(self.jobTrace[job_index]["run"])
            + " ",
            4,
        )
