...
class Backfill:
	...
	def reset(
		self, mode=None, ad_mode=None, node_module=None, debug=None, para_list=None
	):
	...
	def backfill_EASY(self):
		...
		while i < job_num:
			backfill_test = 0
			backfill_test = self.node_module.pre_avail(
				self.wait_job[i]["proc"],
